...
class CustomSignupForm(UserCreationForm):
    ...
    def __init__(self, request=None, *args, **kwargs):
        ...
        self.fields["subdomain"].widget.attrs.update(
            {"class": "form-control", "placeholder": "Enter your subdomain"}
        )
    ...
    def save_tenant(self, user: User):
        ...
        tenant = Tenant(
            schema_name=subdomain, name=f"{user.username}'s Blog", owner=user
        )
        tenant.save()
        return tenant
    ...
    def save_domain(self, tenant):
        subdomain = self.cleaned_data["subdomain"]
        domain = Domain(
            domain=f"{subdomain}.{self.request.get_host().split(':')[0]}", tenant=tenant
        )
        domain.save()
